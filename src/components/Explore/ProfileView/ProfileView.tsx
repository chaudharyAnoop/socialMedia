...
import { fetchSearchResults } from "../../../redux/slices/exploreSlice";

import styles from "./ProfileView.module.css";
import { Account, Post } from "../../../interfaces/explore.interface";
...
const ProfileView: React.FC = () => {
  ...
  const { searchResults, isSearchingLoading, error } = useSelector(
  ...
  useEffect(() => {
    const profile = searchResults.accounts.find(
    ...
    const duplicateIds = postIds.filter(
    ...
    if (profile) {
      setUserProfile({
        ...
        profilePicture:
          profile.profilePicture || `https://i.pravatar.cc/150?u=${profile.id}`,
      });
    } else {
      setUserProfile(null);
    }
    ...
    <div className={styles.profileContainer}>
      ...
      <div className={styles.profileHeader}>
        <img
          ...
          onError={(e) => {
            console.error(
              `Failed to load profile picture for ${userProfile.username}`
            );
            ...
            >
              {post.isVideo ? (
                ...
                <img
                  ...
                  onError={(e) => {
                    console.error(
                      `Failed to load post media: ${post.imageUrl}`
                    );
